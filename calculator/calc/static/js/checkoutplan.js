...
document.getElementById("payform1").addEventListener("submit", async function (event) {
...
try {
    ...
    let response = await fetch(`/checkoutplan/${transactionId}`, {
        method: "POST",
        body: formData
    });
    ...
    if (data.order_id) {
        const detailurl = document.getElementById("payform1").getAttribute("data-detail-url");
        // const redirecturl = document.getElementById("payform").getAttribute("data-redirect-url");
        ...
        var options = {
            ...
            "name": "eHealth",
            "description": "Membership Plan Booking",
            "prefill": {
            ...
            "handler": async function (response) {
                ...
                let confirmResponse = await fetch(`/plan_payment_success/${transactionId}`, {
                    method: "POST",
            ...
            "modal": {
                        "escape": false,
                        "ondismiss": function () {
                            alert("Payment was not completed. Redirecting to Home...");
                            window.location.href =
                                homeUrl; // Redirect to the cart page
                        }
                    }
        };
